...
//
// Ported to RikkaLLM: com.alibaba.mls.api (ApplicationProvider / ModelDownloadManager)
// replaced with the local AppContext helper; remote model download lookup removed.
    ...
    companion object {
        ...
        fun getDefaultConfigFile(modelId:String):String? {
            ...
            if (modelId.startsWith("Builtin/")) {
                ...
                val builtinModelsDir = File(AppContext.get().filesDir, ".mnnmodels/builtin")
            ...
            // Remote model downloads are not part of this module; callers are expected
            // to manage model files themselves and use local/ model ids.
            return null
        }
        ...
        fun getModelConfigDir(modelId: String): String {
            val rootCacheDir =
                AppContext.get().filesDir.toString() + "/configs/" + ModelUtils.safeModelId(
                    modelId
                )
            return rootCacheDir
        }
