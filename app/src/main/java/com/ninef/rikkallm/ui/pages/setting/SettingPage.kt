...
import com.ninef.rikkallm.data.datastore.isNotConfigured
...
import com.ninef.rikkallm.ui.components.ui.icons.DiscordIcon
import com.ninef.rikkallm.ui.components.ui.icons.TencentQQIcon
import com.ninef.rikkallm.ui.context.LocalNavController
import com.ninef.rikkallm.ui.context.Navigator
import com.ninef.rikkallm.ui.hooks.rememberColorMode
import com.ninef.rikkallm.ui.theme.ColorMode
import com.ninef.rikkallm.ui.theme.CustomColors
import com.ninef.rikkallm.utils.joinQQGroup
import com.ninef.rikkallm.utils.openUrl
import com.ninef.rikkallm.utils.plus
...
fun SettingPage(vm: SettingVM = koinViewModel()) {
    ...
    // 赞助提醒已临时隐藏（代码保留），如需恢复显示，恢复下方 if 条件即可
    if (false) {
        AlertDialog(
            onDismissRequest = {
        ...
        LazyColumn(
            ...
            item("modelServices") {
                CardGroup(
                    ...
                    item(
                        onClick = { navController.navigate(Screen.SettingCron) },
                        leadingContent = { Icon(HugeIcons.Sun01, null) },
                        supportingContent = { Text(stringResource(R.string.setting_page_cron_desc)) },
                        headlineContent = { Text(stringResource(R.string.setting_page_cron)) },
                    )
                    item(
                        onClick = { navController.navigate(Screen.SettingWebMount) },
                        leadingContent = { Icon(HugeIcons.LanguageCircle, null) },
                        supportingContent = { Text("将 GitHub 等站点以可读写工具挂载给 Agent") },
                        headlineContent = { Text("网页挂载") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.SettingCliSeat) },
                        leadingContent = { Icon(HugeIcons.ComputerTerminal01, null) },
                        supportingContent = { Text("把 proot 内的 Claude Code / Gemini CLI 等作为席位纳入模型议会") },
                        headlineContent = { Text("外部 CLI 席位") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.DeepRead) },
                        leadingContent = { Icon(HugeIcons.BookOpen01, null) },
                        supportingContent = { Text("基于材料由 LLM 生成结构化深度阅读报告") },
                        headlineContent = { Text("深度阅读") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.ScreenAutomation) },
                        leadingContent = { Icon(HugeIcons.SmartPhone01, null) },
                        supportingContent = { Text("开启无障碍服务后，Agent 可读取并操控屏幕") },
                    ...
                    item(
                        onClick = { navController.navigate(Screen.SettingIde) },
                        leadingContent = { Icon(HugeIcons.Code, null) },
                        supportingContent = { Text("字体、行号、自动换行与默认工作区等编辑器偏好") },
                        headlineContent = { Text("IDE 设置") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.ModelMarket) },
                        leadingContent = { Icon(HugeIcons.Package, null) },
                        supportingContent = { Text("从 HuggingFace 浏览、筛选并一键加载模型") },
                        headlineContent = { Text("模型市场") },
                    )
                    item(
                        onClick = { navController.navigate(Screen.SettingUpdate) },
                        headlineContent = { Text("更新设置") },
                        supportingContent = { Text("配置更新渠道并查看版本与更新") },
                    )
                }
            }
            ...
            item("toolApproval") {
                CardGroup(
                    modifier = Modifier.padding(horizontal = Spacing.sm),
                    title = { Text(stringResource(R.string.setting_page_tool_approval)) },
                ) {
                    item(
                        leadingContent = { Icon(HugeIcons.Alert01, null) },
                        headlineContent = { Text(stringResource(R.string.setting_page_auto_approve_tools)) },
                        supportingContent = { Text(stringResource(R.string.setting_page_auto_approve_tools_desc)) },
                        trailingContent = {
                            Switch(
                                checked = settings.autoApproveTools,
                                onCheckedChange = { checked ->
                                    vm.updateSettings(settings.copy(autoApproveTools = checked))
                                },
                            )
                        },
                    )
                    item(
                        leadingContent = { Icon(HugeIcons.Alert01, null) },
                        headlineContent = { Text(stringResource(R.string.setting_page_auto_approve_high_risk_tools)) },
                        supportingContent = { Text(stringResource(R.string.setting_page_auto_approve_high_risk_tools_desc)) },
                        trailingContent = {
                            Switch(
                                checked = settings.autoApproveHighRiskTools,
                                onCheckedChange = { checked ->
                                    vm.updateSettings(settings.copy(autoApproveHighRiskTools = checked))
                                },
                            )
                        },
                    )
            ...
            item("graphContext") {
                CardGroup(
                    ...
                    item(
                        ...
                        headlineContent = { Text(stringResource(R.string.setting_page_use_graph_context)) },
                        supportingContent = {
                            Text(stringResource(R.string.setting_page_use_graph_context_summary))
                        },
                        trailingContent = {
                            Switch(
                                checked = settings.useGraphContext,
                                onCheckedChange = { checked ->
                                    vm.updateSettings(settings.copy(useGraphContext = checked))
                                },
                            )
                        },
                    )
                    ...
                    item(
                        ...
                        trailingContent = {
                            Row(
                                ...
                                IconButton(
                                    onClick = {
                                        context.openUrl("https://discord.gg/WjW7Eeqr")
                                    }
                                ) {
                                    Icon(
...
private val QQ_GROUPS = listOf(
    QQGroup("RikkaHub 官方群", "945057252"),
)
