...
import com.ninef.rikkallm.ui.components.ui.permission.PermissionNotification
...
fun SettingLocalEnginePage() {
    ...
    val sourceManager: ModelSourceManager = koinInject()
    val marketSource by sourceManager.source.collectAsStateWithLifecycle()
    val recommendedSource by sourceManager.recommended.collectAsStateWithLifecycle()
    val probingSource by sourceManager.probing.collectAsStateWithLifecycle()
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                Text(
                    "本地模型引擎可在设备端离线运行 AI 模型，保护隐私且无需联网推理。可下载官方推荐的 MNN 模型，或浏览在线模型社区获取更多模型。国内用户若访问 Hugging Face 较慢，可在下方「模型源」中切换为魔搭社区（ModelScope）。",
            ...
            item {
                CardGroup(
                    ...
                    title = { Text("模型源（下载与浏览）") },
                ) {
                    item(
                        headlineContent = { Text("选择模型源") },
                        supportingContent = {
                            Column(verticalArrangement = Arrangement.spacedBy(Spacing.sm)) {
                                Text(
                                    "Hugging Face：全球最大的开源模型社区，模型最全，但国内访问可能较慢。魔搭社区：阿里达摩院推出的国内模型社区，国内访问速度快、稳定。选择「自动」会根据你的网络环境智能推荐访问速度更快的源。",
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                                )
                                ModelSourceSelector(
                                    current = marketSource,
                                    onSelect = { sourceManager.setSource(it) },
                                )
                                if (probingSource) {
                                    Text(
                                        "正在检测网络以推荐最佳模型源…",
            ...
            item {
                CardGroup(
                    ...
                    item(
                        onClick = { navigator.navigate(Screen.ModelMarket) },
                        headlineContent = { Text("浏览全部模型") },
                        supportingContent = {
                            Text(
                                "打开在线模型社区，按任务类型、框架、参数量筛选并一键配置。" +
                                    "当前源：${marketSource.label}" +
                                    if (marketSource.isAuto) "（自动 → ${recommendedSource.label}）" else "",
                            )
                        },
                        trailingContent = {
