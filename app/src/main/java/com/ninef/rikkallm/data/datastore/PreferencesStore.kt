...
import com.ninef.rikkallm.data.ai.mcp.McpServerConfig
import com.ninef.rikkallm.data.ai.prompts.DEFAULT_COMPRESS_PROMPT
import com.ninef.rikkallm.data.ai.prompts.DEFAULT_OCR_PROMPT
import com.ninef.rikkallm.data.ai.prompts.DEFAULT_SUGGESTION_PROMPT
import com.ninef.rikkallm.data.ai.prompts.DEFAULT_TITLE_PROMPT
import com.ninef.rikkallm.data.ai.prompts.DEFAULT_TRANSLATION_PROMPT
import com.ninef.rikkallm.data.ai.prompts.LEARNING_MODE_PROMPT
import me.rerere.asr.ASRProviderSetting
import com.ninef.rikkallm.data.datastore.migration.PreferenceStoreV1Migration
import com.ninef.rikkallm.data.datastore.migration.PreferenceStoreV2Migration
import com.ninef.rikkallm.data.datastore.migration.PreferenceStoreV3Migration
...
import com.ninef.rikkallm.data.model.InjectionPosition
import com.ninef.rikkallm.data.model.Lorebook
import com.ninef.rikkallm.data.model.PromptInjection
import com.ninef.rikkallm.data.model.QuickMessage
import com.ninef.rikkallm.data.model.Tag
import com.ninef.rikkallm.data.sync.s3.S3Config
import com.ninef.rikkallm.ui.theme.CustomTheme
import com.ninef.rikkallm.ui.theme.PresetThemes
import com.ninef.rikkallm.utils.JsonInstant
import com.ninef.rikkallm.utils.toMutableStateFlow
...
class SettingsStore(
    ...
    companion object {
        ...
        // 更新渠道：STABLE 仅正式 Release；INCLUDE_DEBUG 同时包含预发布
        ...
        val AUTO_APPROVE_HIGH_RISK_TOOLS = booleanPreferencesKey("auto_approve_high_risk_tools")

        // IDE 内嵌编辑器
        val IDE_WORKSPACE_URI = stringPreferencesKey("ide_workspace_uri")
        val IDE_FONT_SIZE = intPreferencesKey("ide_font_size")
        val IDE_SHOW_LINE_NUMBERS = booleanPreferencesKey("ide_show_line_numbers")
        val IDE_WORD_WRAP = booleanPreferencesKey("ide_word_wrap")
        val IDE_AUTO_SAVE = booleanPreferencesKey("ide_auto_save")
    }
    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                autoApproveHighRiskTools = preferences[AUTO_APPROVE_HIGH_RISK_TOOLS] == true,
                ideWorkspaceUri = preferences[IDE_WORKSPACE_URI] ?: "",
                ideFontSize = preferences[IDE_FONT_SIZE]?.coerceIn(10, 28) ?: 13,
                ideShowLineNumbers = preferences[IDE_SHOW_LINE_NUMBERS] != false,
                ideWordWrap = preferences[IDE_WORD_WRAP] == true,
                ideAutoSave = preferences[IDE_AUTO_SAVE] != false,
                updateChannel = runCatching {
                    UpdateChannel.valueOf(preferences[UPDATE_CHANNEL] ?: UpdateChannel.STABLE.name)
            ...
            settings.copy(
                providers = settings.providers.distinctBy { it.id }.map { provider ->
                    when (provider) {
                        ...
                        is ProviderSetting.LocalServer -> provider.copy(
                            models = provider.models.distinctBy { model -> model.id }
                        )
                    }
                },
                assistants = run {
                    val cleaned = settings.assistants.distinctBy { it.id }.map { assistant ->
                        assistant.copy(
                            // 过滤掉不存在的 MCP 服务器 ID
                    ...
                    // 迁移：移除旧版内置的空配置默认助手，避免列表中显示两个“默认助手”。
                    // 仅当该助手未被用户修改（名称与 systemPrompt 仍为空）且当前默认助手存在时才移除。
                    val hasCurrentDefault = cleaned.any { it.id == DEFAULT_ASSISTANT_ID }
                    cleaned.filterNot { assistant ->
                        assistant.id == OLD_FALLBACK_DEFAULT_ASSISTANT_ID
    ...
    suspend fun update(settings: Settings) {
        ...
        dataStore.edit { preferences ->
            ...
            preferences[AUTO_APPROVE_HIGH_RISK_TOOLS] = settings.autoApproveHighRiskTools
            preferences[IDE_WORKSPACE_URI] = settings.ideWorkspaceUri
            preferences[IDE_FONT_SIZE] = settings.ideFontSize
            preferences[IDE_SHOW_LINE_NUMBERS] = settings.ideShowLineNumbers
            preferences[IDE_WORD_WRAP] = settings.ideWordWrap
            preferences[IDE_AUTO_SAVE] = settings.ideAutoSave
            preferences[UPDATE_CHANNEL] = settings.updateChannel.name
        }
    }
...
data class Settings(
    ...
    // 上下文图谱：开启后生成请求依据图谱（含 prune / merge / 连线编辑）装配上下文，
    // 而非线性 currentMessages；无图谱时自动回退。默认关闭以保留原有生成行为。
    val useGraphContext: Boolean = false,

    // IDE 设置
    val ideWorkspaceUri: String = "",
    val ideFontSize: Int = 13,
    val ideShowLineNumbers: Boolean = true,
    val ideWordWrap: Boolean = false,
    val ideAutoSave: Boolean = true,
...
data class DisplaySetting(
    ...
    val enableDiagramRendering: Boolean = true,
...
// 旧版本曾内置两个默认助手（一个空配置、一个带默认 systemPrompt），二者名称均为空，
// 列表中会显示两个“默认助手”。现精简为单个默认助手；旧空配置默认助手的 ID 仅用于
// 旧数据迁移（见 settingsFlowRaw 中的清理逻辑），不应作为新的默认助手创建。
private val OLD_FALLBACK_DEFAULT_ASSISTANT_ID = Uuid.parse("0950e2dc-9bd5-4801-afa3-aa887aa36b4e")
