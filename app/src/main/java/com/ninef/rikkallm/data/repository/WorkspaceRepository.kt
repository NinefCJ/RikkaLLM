...
class WorkspaceRepository(
    ...
    /**
     * 从外部文件夹（SAF tree uri）创建一个新工作区，并递归复制其文件到工作区的 FILES 区。
     * 复制在 IO 调度器上执行，适用于把用户在系统文件管理器中选择的任意目录导入为可用工作区。
     * 若复制过程中失败，会自动删除已创建的工作区记录，避免产生半成品。
     */
    suspend fun createFromTreeUri(name: String, treeUri: Uri): WorkspaceEntity = withContext(Dispatchers.IO) {
        val workspace = create(name)
        try {
            val tree = DocumentFile.fromTreeUri(context, treeUri)
                ?: throw IllegalStateException("无法读取所选文件夹")
            copyDocumentTree(tree, workspace.id, "")
        } catch (e: Throwable) {
            withContext(NonCancellable) {
                runCatching { delete(workspace.id) }
            }
            throw e
        }
        workspace
    ...
    private suspend fun copyDocumentTree(tree: DocumentFile, workspaceId: String, relativePath: String) {
        for (child in tree.listFiles()) {
            val displayName = child.name ?: continue
            val childRelativePath = if (relativePath.isEmpty()) displayName else "$relativePath/$displayName"
            if (child.isDirectory) {
                copyDocumentTree(child, workspaceId, childRelativePath)
            } else {
                child.uri?.let { uri ->
