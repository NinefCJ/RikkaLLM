package com.ninef.rikkallm.data.files
...
import com.ninef.rikkallm.utils.exportImageFile
...
class FilesManager(
                    ...
                    when (part) {
                        is UIMessagePart.Image -> {
                            if (part.url.startsWith("data:image")) {
                                ...
                                val localUrl = urls.firstOrNull()?.toString()
                                part.copy(
                                    url = localUrl ?: part.url,
                                )
                            } else {
                                part
                            }
    ...
    suspend fun saveMessageImage(activityContext: Context, image: String) = withContext(Dispatchers.IO) {
        val activity = activityContext.getActivity() ?: return@withContext
        when {
